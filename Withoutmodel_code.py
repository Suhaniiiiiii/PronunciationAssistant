...
#from phonemizer import phonemize # Added for phoneme matching
#from phonemizer.separator import Separator # Added for phoneme matching
#from g2p_en import G2p  # Import g2p-en for phoneme conversion

class PronunciationDetector:
    def __init__(self, root, username):
        self.root = root
        self.username = username
        self.root.title("Pronunciation Mistake Detector")
        self.root.geometry("850x650")
        self.root.configure(bg='#e0f7fa')  # Light cyan background for a clean look

        self.current_sentence = ""
        self.logged_in = True
        self.user_data = {"correct_count": 0, "incorrect_count": 0}
        
        #self.g2p = G2p()  # Initialize g2p-en
    ...
    def record_sentence(self):
        ...
        with sr.AudioFile(audio_file) as source:
            try:
                ...
                transcribed_phonemes = self.get_phonemes(transcribed_text)  # Added phoneme conversion
                expected_phonemes = self.get_phonemes(expected_text)  # Added phoneme conversion
                '''

                # Compare phonemes
                if transcribed_text == expected_text:  # Updated comparison to phoneme-level
                    result = "Correct pronunciation!"
                    self.user_data["correct_count"] += 1
                else:
                    result = "Incorrect pronunciation."
                    self.user_data["incorrect_count"] += 1
                    self.speak_correct_pronunciation()  # Added for verbal feedback
                ...
                self.label.config(text="Sorry, there was an error with the request.")
    ...
    def apply_theme(self, theme_name):
        if theme_name == "Dark":
            ...
            self.label.configure(fg='white', bg='#2e2e2e')
            self.custom_sentence_entry.configure(bg='#1f1f1f', fg='white')
            self.result_text.configure(bg='#1f1f1f', fg='white')
            self.button_frame.configure(bg='#2e2e2e')
        elif theme_name == "Light":
            self.root.configure(bg='#d3d3d3')
            self.label.configure(fg='black', bg='#d3d3d3')
            self.custom_sentence_entry.configure(bg='white', fg='black')
            self.result_text.configure(bg='white', fg='black')
            self.button_frame.configure(bg='#d3d3d3')
